...
nfc_context_new(void)
{
  ...
  int it;
  for (it = 0; it < MAX_USER_DEFINED_DEVICES; it++) {
    strcpy(res->user_defined_devices[it].name, "");
    strcpy(res->user_defined_devices[it].connstring, "");
    res->user_defined_devices[it].optional = false;
  }
  ...
  uint32_t i;
  for (i = 0; i < res->user_defined_device_count; i++) {
