...
pn53x_writeback_register(struct nfc_device *pnd)
{
  ...
  size_t n2;
  for (n2 = 0; n2 < PN53X_CACHE_REGISTER_SIZE; n2++) {
    if (CHIP_DATA(pnd)->wb_mask[n2] == 0xff) {
      const uint16_t pn53x_register_address = PN53X_CACHE_REGISTER_MIN_ADDRESS + n2;
      ...
      BUFFER_APPEND(abtWriteRegisterCmd, CHIP_DATA(pnd)->wb_data[n2]);
      // This register is handled, we reset the mask to prevent
      CHIP_DATA(pnd)->wb_mask[n2] = 0x00;
    }
  }
                            ...
                            nfc_target *pnt)
{
  ...
  if (CHIP_DATA(pnd)->type == PN532) {
    ...
    size_t n3;
    for (n3 = 0; n3 < szModulations; n3++) {
      const pn53x_target_type ptt = pn53x_nm_to_ptt(pnmModulations[n3]);
    ...
    do {
      size_t p;
      for (p = 0; p < uiPollNr; p++) {
        size_t n4;
        for (n4 = 0; n4 < szModulations; n4++) {
          uint8_t *pbtInitiatorData;
          size_t szInitiatorData;
          prepare_initiator_data(pnmModulations[n4], &pbtInitiatorData, &szInitiatorData);
          const int timeout_ms = uiPeriod * 150;

          if ((res = pn53x_initiator_select_passive_target_ext(pnd, pnmModulations[n4], pbtInitiatorData, szInitiatorData, pnt, timeout_ms)) < 0) {
                 ...
                 const uint8_t btPollNr, const uint8_t btPeriod, nfc_target *pntTargets, const int timeout)
{
  ...
  size_t n5;
  for (n5 = 0; n5 < szTargetTypes; n5++) {
    abtCmd[3 + n5] = ppttTargetTypes[n5];
  }
...
pn53x_build_frame(uint8_t *pbtFrame, size_t *pszFrame, const uint8_t *pbtData, const size_t szData)
{
  if (szData <= PN53x_NORMAL_FRAME__DATA_MAX_LEN) {
    ...
    size_t szPos1;
    for (szPos1 = 0; szPos1 < szData; szPos1++) {
      btDCS -= pbtData[szPos1];
    }
    ...
    uint8_t btDCS = (256 - 0xD4);
    size_t szPos2;
    for (szPos2 = 0; szPos2 < szData; szPos2++) {
      btDCS -= pbtData[szPos2];
    }
...
pn53x_get_information_about(nfc_device *pnd, char **pbuf)
{
  ...
  *pbuf = malloc(buflen);
  ...
  int i9;
  for (i9 = 0; nmt[i9]; i9++) {
    if ((res = snprintf(buf, buflen, "%s%s (", (i9 == 0) ? "" : ", ", str_nfc_modulation_type(nmt[i9]))) < 0) {
      free(*pbuf);
      return NFC_ESOFT;
    }
    buf += res;
    ...
    buflen -= res;
    const nfc_baud_rate *nbr;
    if ((res = nfc_device_get_supported_baud_rate(pnd, nmt[i9], &nbr)) < 0) {
      free(*pbuf);
      return res;
    }
    int j;
  ...
  if ((res = nfc_device_get_supported_modulation(pnd, N_TARGET, &nmt)) < 0) {
    ...
    int i8;
  for (i8 = 0; nmt[i8]; i8++) {
    if ((res = snprintf(buf, buflen, "%s%s (", (i8 == 0) ? "" : ", ", str_nfc_modulation_type(nmt[i8]))) < 0) {
      free(*pbuf);
      return NFC_ESOFT;
    }
    buf += res;
    ...
    buflen -= res;
    const nfc_baud_rate *nbr;
    if ((res = nfc_device_get_supported_baud_rate(pnd, nmt[i8], &nbr)) < 0) {
      free(*pbuf);
      return res;
    }
    int j9;
    for (j9 = 0; nbr[j9]; j9++) {
      if ((res = snprintf(buf, buflen, "%s%s", (j9 == 0) ? "" : ", ", str_nfc_baud_rate(nbr[j9]))) < 0) {
        free(*pbuf);
        return NFC_ESOFT;
      }
      buf += res;
